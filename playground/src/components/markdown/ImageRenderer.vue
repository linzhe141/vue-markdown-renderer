...
const imageLabel = computed(() => props.alt || props.title || "Markdown image");
...
<template>
  <figure
    class="group not-prose my-6 overflow-hidden rounded-2xl border border-gray-200/80 bg-white shadow-sm transition-shadow duration-200 hover:shadow-lg dark:border-white/10 dark:bg-[#111111]"
  >
    <button
      type="button"
      class="group block w-full cursor-zoom-in overflow-hidden bg-[linear-gradient(180deg,#f8fafc_0%,#eef2f7_100%)] text-left outline-none dark:bg-[linear-gradient(180deg,#111827_0%,#0f172a_100%)]"
      :disabled="!props.src"
      @click="openPreview"
    >
      <img
        ...
        class="max-h-[28rem] w-full object-contain transition duration-300 group-hover:scale-[1.01]"
        loading="lazy"
      />
      <div
        v-else
  ...
  <teleport to="body">
    <transition name="image-preview-fade">
      ...
      >
        <div
          class="relative w-full max-w-6xl overflow-hidden rounded-3xl border border-white/10 bg-[#0b1020] shadow-2xl"
          @click.stop
        >
          <button
            type="button"
          ...
          >
            <div class="min-w-0">
              ...
              <div v-if="props.src" class="truncate text-xs text-white/55">
                {{ props.src }}
              </div>
            </div>
